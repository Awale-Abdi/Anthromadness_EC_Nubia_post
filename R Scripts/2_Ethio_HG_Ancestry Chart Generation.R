...
df <- read.csv("2_Horner_Ethio_HG_Ancestry.csv", row.names = 1)
...
p <- plot_ly(
  ...
  layout(
    title = "",
    xaxis = list(title = ""),
    yaxis = list(title = "Ethio-HG Ancestry %")
  )

# Saving the interactive chart as an HTML file in the working directory
saveWidget(as_widget(p), "2_Ethio_HG_Ancestry_Chart.html", selfcontained = TRUE)
