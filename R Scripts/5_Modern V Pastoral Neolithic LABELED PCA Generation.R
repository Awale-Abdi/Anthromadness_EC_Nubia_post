...
modern_cushite_ethiosemite_samples <- c("Somali", "Tigrinya", "Oromo", "Afar", "Saho", "Agaw", "Beta-Israel", "Amhara")
...
p <- ggplot(pca_data, aes(x = PC1, y = PC2, color = Group, shape = Group)) +
  ...
  geom_text_repel(aes(label = Sample), show.legend = FALSE, size = 3, 
                  ...
                  segment.color = NA, max.overlaps = 50) +  # Increased max.overlaps to allow more labels
...
output_filename <- "5_ModernVPastoral_Labeled_PCA.jpg"
