...
df <- read.csv("8_Ma'rib_Yemeni_Results.csv", check.names = FALSE)
...
heatmap_table <- datatable(
  ...
  options = list(
    ...
    columnDefs = list(
      list(
        # 0-based indices for DataTables: color all but the first column
        targets = 1:(ncol(df) - 1),
        render = JS(
...
saveWidget(
  heatmap_table,
  "8_Ma'rib_Yemeni_V_Egyptian_Heatmap.html",
  selfcontained = TRUE
)
