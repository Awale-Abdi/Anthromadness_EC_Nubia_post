...
heatmap_table <- datatable(
  ...
  options = list(
    ...
    columnDefs = list(
      list(
        # same visible columns as original (Dinka onward),
        # but 0-based indices to avoid the warning
        targets = 1:(ncol(df) - 1),
        render = JS(
...
# Saving the table/heatmap as an HTML file in the working directory
saveWidget(
  heatmap_table,
  "8_Naqab_Bedouin_B_V_Egyptian_Heatmap.html",
  selfcontained = TRUE
)
