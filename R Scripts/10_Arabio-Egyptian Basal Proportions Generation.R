...
# Loading the dataset
df <- read.csv("10_Arabio-Egyptian-Basal-Proportions_Manually_Averaged.csv",
...
# Reversing the levels so that, after coord_flip(), the plot is top-to-bottom like in the CSV
original_order <- df$Sample
df$Sample <- factor(df$Sample, levels = rev(original_order))

# Converting the dataframe to a long format for ggplot
df_long <- melt(df, id.vars = c("Sample", "distance_percent"),
...
htmlwidgets::saveWidget(interactive_plot,
                        "10_Arabio-Egyptian_Basal_Proportions_Plot.html")
