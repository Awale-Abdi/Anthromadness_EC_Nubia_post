...
data_long <- data %>%
  select(Population, all_of(correct_ancestry_order)) %>%  # Ensure correct order is selected
...
p <- ggplot(data_long, aes(x = Population, y = Percentage, fill = Ancestry, 
                           text = paste("Ancestry:", Ancestry, "<br>Proportion:", Percentage, "%", 
                                        "<br>Distance%:", data$`distance%`[match(Population, data$Population)]))) +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_fill_manual(values = c("blue", "lightblue", "darkred", "red")) +
  labs(title = "", x = "", y = "") +
  theme_minimal()
